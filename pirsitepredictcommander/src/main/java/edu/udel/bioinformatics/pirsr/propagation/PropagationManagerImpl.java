...
import edu.udel.bioinformatics.pirsr.uniprot.io.UniProtFlatFileParser;
 ...
 * <br>
 * 
 * The implementation of <code>PropagationManager</code>.
 */
public class PropagationManagerImpl implements PropagationManager {
	...
	public PropagationManagerImpl(PropagationDataFactory propagationDataFactory, PIRRuleManager pirRuleManager) {
		super();
	...
	public Alignment getAlignment(String ruleAC, String entryId, String templateAC) {
	...
	public Alignment getAlignmentByEntryAndRule(String entryId, String ruleAC) {
		for (Alignment alignment : alignmentList) {
			if (alignment.getEntryId().equals(entryId) && alignment.getRuleAC().equals(ruleAC)) {
				return alignment;
			}
		}
		return null;
	...
	public void propagate(String propagationDirectory) {
		...
		List<PIRRule> ruleList = this.pirRuleManager.parsePIRRules(ruleFile, PIRRuleManager.Format.UNIRULE, false, "Source");
		if (pirRuleManager.getParserLogs() != null && pirRuleManager.getParserLogs().length() > 0) {
			System.err.println(pirRuleManager.getParserLogs());
			System.err.println("\"" + ruleFile + "\" has syntax error, program quit.\n");
			System.exit(1);
		}
		// if (debug) {
		...
		for (PIRRule rule : ruleList) {
			...
			for (UniProtEntry entry : entryList) {
				...
				Alignment alignment = this.getAlignmentByEntryAndRule(entryID, ruleAC);
				...
				PIRRuleUtil pirruleUtil = this.pirRuleManager.getDataFactory().buidPIRRuleUtil(rule);
				List<ControlBlock> scopeControlBlocks = pirruleUtil.getScopeBlockControlBlocks();
				if (scopeControlBlocks == null || scopeControlBlocks.size() == 0) {
					if (!PropagationUtil.checkNonControlBlockScope(rule, entry)) {
						note += "Taxonomic scope does not match; ";
					}
				}
				if (note.length() > 0) {
					PropagationRecord propagationRecord = this.propagateionDataFactory.buildPropagationRecord();
					EntryAnnotation entryAnnotation = this.propagateionDataFactory.buildEntryAnnotation(entryAC, entryID, entry.getEntryType());
					RuleAnnotation ruleAnnotation = this.propagateionDataFactory.buildRuleAnnotation(ruleAC, templateAC);
					...
					// rule));
					propagationResults.addAll(propagateEntry(alignment, entry, rule));
				}
				propagationReports.add(generateReport(entry, rule, propagationResults));
			}
			printReports(rule, propagationReports, propagationDirectory, swissProtEntryCount, tremblEntryCount);
			// if (debug) {
	...
	private void checkTemplateInDatFile(List<PIRRule> ruleList, List<UniProtEntry> entryList) {
		for (PIRRule rule : ruleList) {
			String templateAC = rule.getTemplateAC();
			boolean found = false;
			for (UniProtEntry entry : entryList) {
			...
			if (!found) {
				System.err
						.println("Template entry " + templateAC + " for rule " + rule.getRuleAC() + " does not exist in .dat file, program quit.\n");
				System.exit(1);
			}
	...
	// private void checkTemplate(List<PIRRule> ruleList, List<UniProtEntry>
	// entryList) {
	// for(PIRRule rule: ruleList) {
	// String templateAC = rule.getTemplateAC();
	// for()
	...
	private void printReports(PIRRule rule, List<PropagationReport> propagationReports, String propagationDirectory, int swissProtEntryCount,
		...
		for (PropagationReport report : propagationReports) {
			...
			List<PropagationRecord> swissProtkrakenRecords = report.getSwissProtKrakenRecord();
			if (swissProtkrakenRecords != null && swissProtkrakenRecords.size() > 0) {
				allSwissProtKrakenRecords.addAll(swissProtkrakenRecords);
			}

			List<PropagationRecord> tremblkrakenRecords = report.getTremblKrakenRecord();
			if (tremblkrakenRecords != null && tremblkrakenRecords.size() > 0) {
				allTremblKrakenRecords.addAll(tremblkrakenRecords);
			}
			if (report.getOtherAnnotations() != null && report.getOtherAnnotations().length() > 0) {
				otherAnnotations.append(report.getOtherAnnotations() + "\n");
			}
			if (report.getSwissProtTruePositiveAnnotations() != null && report.getSwissProtTruePositiveAnnotations().length() > 0) {
				swissProtTruePositiveAnnotations.append(report.getSwissProtTruePositiveAnnotations() + "\n");
			}
			if (report.getSwissProtFalsePositiveAnnotations() != null && report.getSwissProtFalsePositiveAnnotations().length() > 0) {
				swissProtFalsePositiveAnnotations.append(report.getSwissProtFalsePositiveAnnotations() + "\n");
			}
			if (report.getSwissProtFalseNegativeAnnotations() != null && report.getSwissProtFalseNegativeAnnotations().length() > 0) {
				swissProtFalseNegativeAnnotations.append(report.getSwissProtFalseNegativeAnnotations() + "\n");
			}

			if (report.getTremblTruePositiveAnnotations() != null && report.getTremblTruePositiveAnnotations().length() > 0) {
				tremblTruePositiveAnnotations.append(report.getTremblTruePositiveAnnotations() + "\n");
			}
			if (report.getTremblFalsePositiveAnnotations() != null && report.getTremblFalsePositiveAnnotations().length() > 0) {
				tremblFalsePositiveAnnotations.append(report.getTremblFalsePositiveAnnotations() + "\n");
			}
			if (report.getTremblFalseNegativeAnnotations() != null && report.getTremblFalseNegativeAnnotations().length() > 0) {
				tremblFalseNegativeAnnotations.append(report.getTremblFalseNegativeAnnotations() + "\n");
			}
		}
		...
		/* confidence */
		double c = (p + z * z / (2 * n) - z * Math.sqrt(p / n - p * p / n + z * z / (4 * n * n))) / (1 + z * z / n);

		List<PropagationRecord> finalSwissProtKrakenRecords = addPrecisionAndConfidence(allSwissProtKrakenRecords, p, c);
		...
		for (PropagationRecord swissProtKrakenRecord : finalSwissProtKrakenRecords) {
			if (!swissProtKrakenACs.contains(swissProtKrakenRecord.getEntryAnnotation().getEntryAC())) {
				swissProtKrakenACs.add(swissProtKrakenRecord.getEntryAnnotation().getEntryAC());
			}
			swissProtKrakenAnnotationCount++;
			finalSwissProtKrakenAnnotations.append(swissProtKrakenRecord.toKraken() + "\n");
		}
		int swissProtKrakenACCount = swissProtKrakenACs.size();

		List<PropagationRecord> finalTremblKrakenRecords = addPrecisionAndConfidence(allTremblKrakenRecords, p, c);
		...
		for (PropagationRecord tremblKrakenRecord : finalTremblKrakenRecords) {
			if (!tremblKrakenACs.contains(tremblKrakenRecord.getEntryAnnotation().getEntryAC())) {
				tremblKrakenACs.add(tremblKrakenRecord.getEntryAnnotation().getEntryAC());
			}
		...
		if (debug) {
			System.out.println("Total swissprot entries: " + swissProtKrakenACCount);
			System.out.println("Total swissprot annotations: " + swissProtKrakenAnnotationCount);
			...
			System.out.println("Total trembl annotations: " + tremblKrakenAnnotationCount);
		...
		String summary = "";
		summary += ruleAC + "\t" + truePositiveCount + "\t" + falsePositiveCount + "\t" + "0\t" + falseNegativeCount + "\t"
				+ (truePositiveCount + falsePositiveCount) + "\t" + String.format("%.2f", p) + "\t" + String.format("%.2f", c) + "\t"
				+ swissProtEntryCount + "\t" + tremblEntryCount + "\t" + (swissProtEntryCount + tremblEntryCount) + "\t"
				+ (swissProtKrakenACCount + tremblKrakenACCount);
		// if (debug) {
		System.out.println("\n" + summary + "\n");
		// }

		String swissProtTruePositiveAnnotationsFile = propagationDirectory + System.getProperty("file.separator") + ruleAC + "_SP_TP.txt";
		writeToFile(swissProtTruePositiveAnnotationsFile, swissProtTruePositiveAnnotations.toString());
		String swissProtFalsePositiveAnnotationsFile = propagationDirectory + System.getProperty("file.separator") + ruleAC + "_SP_FP.txt";
		writeToFile(swissProtFalsePositiveAnnotationsFile, swissProtFalsePositiveAnnotations.toString());
		String swissProtFalseNegativeAnnotationsFile = propagationDirectory + System.getProperty("file.separator") + ruleAC + "_SP_FN.txt";
		writeToFile(swissProtFalseNegativeAnnotationsFile, swissProtFalseNegativeAnnotations.toString());

		String tremblTruePositiveAnnotationsFile = propagationDirectory + System.getProperty("file.separator") + ruleAC + "_TR_TP.txt";
		writeToFile(tremblTruePositiveAnnotationsFile, tremblTruePositiveAnnotations.toString());
		String tremblFalsePositiveAnnotationsFile = propagationDirectory + System.getProperty("file.separator") + ruleAC + "_TR_FP.txt";
		writeToFile(tremblFalsePositiveAnnotationsFile, tremblFalsePositiveAnnotations.toString());
		String tremblFalseNegativeAnnotationsFile = propagationDirectory + System.getProperty("file.separator") + ruleAC + "_TR_FN.txt";
		writeToFile(tremblFalseNegativeAnnotationsFile, tremblFalseNegativeAnnotations.toString());

		String otherAnnotationsFile = propagationDirectory + System.getProperty("file.separator") + ruleAC + "_others.txt";
		writeToFile(otherAnnotationsFile, otherAnnotations.toString());

		String swissProtKrakenFile = propagationDirectory + System.getProperty("file.separator") + ruleAC + "_SP_kraken.txt";
		writeToFile(swissProtKrakenFile, finalSwissProtKrakenAnnotations.toString());

		String tremblKrakenFile = propagationDirectory + System.getProperty("file.separator") + ruleAC + "_TR_kraken.txt";
		writeToFile(tremblKrakenFile, finalTremblKrakenAnnotations.toString());
		String summaryFile = propagationDirectory + System.getProperty("file.separator") + ruleAC + "_summary.txt";
	...
	private List<PropagationRecord> addPrecisionAndConfidence(List<PropagationRecord> allKrakenRecords, double precision, double confidence) {
	...
	private PropagationReport generateReport(UniProtEntry entry, PIRRule rule, List<PropagationRecord> propagationResults) {
		...
		header += rule.getRuleAC() + " | " + rule.getTrigger() + " | " + rule.getTemplateAC();
		if (debug) {
		...
		for (PropagationRecord propagationRecord : propagationResults) {
			...
			EntryAnnotation entryAnnotation = propagationRecord.getEntryAnnotation();
			RuleAnnotation ruleAnnotation = propagationRecord.getRuleAnnotation();
			...
			if (propagationRecord.getPropagationStats().equals(PropagationStats.TP)) {
				if (entryAnnotation.getEntryType().equals(EntryType.SWISSPROT)) {
					swissProtTruePositivesBuffer.append(propagationRecord.toReport() + "\n");
					if (!swissProtTruePositiveAnnotations.contains(entryAnnotation.toReport() + "|" + ruleAnnotation.toReport())) {
						swissProtTruePositiveAnnotations.add(entryAnnotation.toReport() + "|" + ruleAnnotation.toReport());
					}
				} else {
					tremblTruePositivesBuffer.append(propagationRecord.toReport() + "\n");
					if (propagationRecord.isPropagated()) {
						if (!tremblKrakenAnnotations.contains(entryAnnotation.toKraken(ruleAnnotation))) {
							tremblKrakenAnnotations.add(entryAnnotation.toKraken(ruleAnnotation));
							tremblKrakenReports.add(propagationRecord);
						}
					}
				}
			} else if (propagationRecord.getPropagationStats().equals(PropagationStats.FP)) {
				if (entryAnnotation.getEntryType().equals(EntryType.SWISSPROT)) {
					swissProtFalsePositivesBuffer.append(propagationRecord.toReport() + "\n");
					if (!swissProtFalsePositiveAnnotations.contains(entryAnnotation.toReport() + "|" + ruleAnnotation.toReport())) {
						swissProtFalsePositiveAnnotations.add(entryAnnotation.toReport() + "|" + ruleAnnotation.toReport());
					}
					if (propagationRecord.isPropagated()) {
						if (!swissProtKrakenAnnotations.contains(entryAnnotation.toKraken(ruleAnnotation))) {
							swissProtKrakenAnnotations.add(entryAnnotation.toKraken(ruleAnnotation));
							swissProtKrakenReports.add(propagationRecord);
						}
					}
				} else {
					tremblFalsePositivesBuffer.append(propagationRecord.toReport() + "\n");
					if (propagationRecord.isPropagated()) {
						if (!tremblKrakenAnnotations.contains(entryAnnotation.toKraken(ruleAnnotation))) {
							tremblKrakenAnnotations.add(entryAnnotation.toKraken(ruleAnnotation));
							tremblKrakenReports.add(propagationRecord);
			...
			} else if (propagationRecord.getPropagationStats().equals(PropagationStats.FN)) {
				if (entryAnnotation.getEntryType().equals(EntryType.SWISSPROT)) {
					swissProtFalseNegativesBuffer.append(propagationRecord.toReport() + "\n");
					if (!swissProtFalseNegativeAnnotations.contains(entryAnnotation.toReport() + "|" + ruleAnnotation.toReport())) {
						swissProtFalseNegativeAnnotations.add(entryAnnotation.toReport() + "|" + ruleAnnotation.toReport());
					}
				} else {
					tremblFalseNegativesBuffer.append(propagationRecord.toReport() + "\n");
				}
			} else {
		...
		report.setSwissProtFalseNegativeCount(swissProtFalseNegativeAnnotations.size());
		...
		report.setSwissProtFalsePositiveCount(swissProtFalsePositiveAnnotations.size());
		if (swissProtTruePositivesBuffer.toString().length() > 0) {
			...
			report.setSwissProtTruePositiveCount(swissProtTruePositiveAnnotations.size());
			if (debug) {
		...
		if (debug) {
			System.out.println(entry.getPrimiaryAccessionNumber() + "\t" + swissProtTruePositiveAnnotations.size() + "\t"
					+ swissProtFalsePositiveAnnotations.size() + "\t" + swissProtFalseNegativeAnnotations.size() + "\n");
		}

		return report;
	}

	private List<PropagationRecord> propagateEntry(Alignment alignment, UniProtEntry entry, PIRRule rule) {
		//Map<Integer, Boolean> ftGroupMatches = PropagationUtil.checkFTGroupMatch(entry, alignment, rule);
		FTGroupMatch ftGroupMatchResult = PropagationUtil.getFTGroupMatch(entry, alignment, rule);
		Map<Integer, Boolean> ftGroups = ftGroupMatchResult.getFtGroups();
		String ftGroupMatchNote = ftGroupMatchResult.getNote();
		ExpressionEvaluator expressionEvaluator = new ExpressionEvaluator(entry, rule, ftGroups);

		List<PropagationRecord> entryPropagationResults = new ArrayList<PropagationRecord>();
		if (debug) {
			System.out.println("\n" + entry.getPrimiaryAccessionNumber() + " | " + entry.getEntryId() + " | " + entry.getEntryType() + " | "
					+ rule.getRuleAC() + " | " + rule.getTrigger() + " | " + rule.getTemplateAC());
		}
		...
		if (debug) {
			System.out.println(entry.getPrimiaryAccessionNumber() + "| " + entry.getEntryId() + " | " + ftGroups + "\n\n");
		}
			...
			for (ControlBlock controlBlock : controlBlocks) {
				...
				for (ControlStatement controlStatement : controlStatements) {
					if (!(controlStatement instanceof EndCaseStatement)) {
						ExpressionValue controlStatementEval = expressionEvaluator.evaluateExpression(controlStatement.getExpression());
						...
						for (Line ruleLine : applicableLines) {
							if (ruleLine instanceof CCLine) {
								...
								propagationRecord = propagateCCLine(alignment, entry, rule, ruleLine, controlStatement, controlStatementEval,
										ftGroupMatchResult);
							} else if (ruleLine instanceof FeatureDescriptionLine) {
								propagationRecord = propagateFeatureDescriptionLine(alignment, entry, rule, ruleLine, controlStatement,
										controlStatementEval, ftGroupMatchNote, ftGroups);
							} else if (ruleLine instanceof KWLine) {
								propagationRecord = propagateKWLine(alignment, entry, rule, ruleLine, controlStatement, controlStatementEval,
										ftGroupMatchResult);
							}
		...
		entryPropagationResults = checkFalseNegatives(entry, rule, alignment, entryPropagationResults);

		return entryPropagationResults;
	}

	private List<PropagationRecord> checkFalseNegatives(UniProtEntry entry, PIRRule rule, Alignment alignment,
			List<PropagationRecord> entryPropagationResults) {
		List<PropagationRecord> entryPropagationResultsAfterCheckFalseNegatives = checkKeywordFalseNegatives(entry, rule, entryPropagationResults);

		entryPropagationResultsAfterCheckFalseNegatives = checkCommentFalseNegatives(entry, rule, entryPropagationResultsAfterCheckFalseNegatives);

		entryPropagationResultsAfterCheckFalseNegatives = checkFeatureTableFalseNegatives(entry, rule,
				entryPropagationResultsAfterCheckFalseNegatives);

		return entryPropagationResultsAfterCheckFalseNegatives;
	}

	private List<PropagationRecord> checkKeywordFalseNegatives(UniProtEntry entry, PIRRule rule, List<PropagationRecord> entryPropagationResults) {
		...
		for (PropagationRecord propagationRecord : entryPropagationResults) {
			if (propagationRecord.getEntryAnnotation() != null) {
				if (propagationRecord.getEntryAnnotation() instanceof EntryKeywordAnnotation) {
					EntryKeyword keyword = ((EntryKeywordAnnotation) propagationRecord.getEntryAnnotation()).getKeyword();
					if (keyword != null) {
		...
		for (EntryKeyword keyword : entryKeywordList) {
			if (!propagatedKeywordList.contains(keyword) && ruleHasKeyword(rule, keyword)) {

				String entryAC = entry.getPrimiaryAccessionNumber();
				String entryID = entry.getEntryId();
				PropagationRecord propagationRecord = this.propagateionDataFactory.buildPropagationRecord();
				RuleKeywordAnnotation ruleKeywordAnnotation = this.propagateionDataFactory.buildRuleKeywordAnnotation(rule.getRuleAC(),
						rule.getTemplateAC(), null);
				EntryKeywordAnnotation entryKeywordAnnotation = this.propagateionDataFactory.buildEntryKeywordAnnotation(entryAC, entryID,
				...
				if (propagationRecord.getNote() != null) {
					propagationRecord.setNote((propagationRecord.getNote() + " Annotation exists in the entry, but is not predicated; ").trim());
				} else {
					propagationRecord.setNote("Annotation exists in the entry, but is not predicated; ");
				}
				entryPropagationResultsAfterCheckFalseNegatives.add(propagationRecord);
			}
		}
	...
	private List<PropagationRecord> checkFeatureTableFalseNegatives(UniProtEntry entry, PIRRule rule, List<PropagationRecord> entryPropagationResults) {
		List<PropagationRecord> entryPropagationResultsAfterCheckFalseNegatives = entryPropagationResults;
		...
		for (PropagationRecord propagationRecord : entryPropagationResults) {
			if (propagationRecord.getEntryAnnotation() != null) {
				if (propagationRecord.getEntryAnnotation() instanceof EntryFeatureTableAnnotation) {
					EntryFeatureTable feature = ((EntryFeatureTableAnnotation) propagationRecord.getEntryAnnotation()).getFeature();
					if (feature != null) {
		...
		for (EntryFeatureTable feature : entryFeatureList) {
			if (!propagatedFeatureList.contains(feature) && ruleHasFeatureDescription(rule, feature)) {

				String entryAC = entry.getPrimiaryAccessionNumber();
				String entryID = entry.getEntryId();
				PropagationRecord propagationRecord = this.propagateionDataFactory.buildPropagationRecord();
				RuleFeatureTableAnnotation ruleFeatureTableAnnotation = this.propagateionDataFactory.buildRuleFeatureTableAnnotation(
						rule.getRuleAC(), rule.getTemplateAC(), null);
				EntryFeatureTableAnnotation entryFeatureTableAnnotation = this.propagateionDataFactory.buildEntryFeatureTableAnnotation(entryAC,
						entryID, entry.getEntryType(), feature, "");
	...
	private boolean ruleHasFeatureDescription(PIRRule rule, EntryFeatureTable feature) {
		for (Line line : rule.getAnnotationSection().getLines()) {
			if (line instanceof FeatureDescriptionLine) {
				FeatureDescriptionLine featureDescriptionLine = (FeatureDescriptionLine) line;
				if (featureDescriptionLine.getFeatureKey().name().equals(feature.getFeatureType().name())) {
					return true;
				}
			}
		}
	...
	private List<PropagationRecord> checkCommentFalseNegatives(UniProtEntry entry, PIRRule rule, List<PropagationRecord> entryPropagationResults) {
		List<PropagationRecord> entryPropagationResultsAfterCheckFalseNegatives = entryPropagationResults;
		...
		for (PropagationRecord propagationRecord : entryPropagationResults) {
			if (propagationRecord.getEntryAnnotation() != null) {
				if (propagationRecord.getEntryAnnotation() instanceof EntryCommentAnnotation) {
					EntryComment comment = ((EntryCommentAnnotation) propagationRecord.getEntryAnnotation()).getComment();
					if (comment != null) {
		...
		for (EntryComment comment : entryCommentList) {
			if (!propagatedCommentList.contains(comment) && ruleHasComment(rule, comment)) {

				String entryAC = entry.getPrimiaryAccessionNumber();
				String entryID = entry.getEntryId();
				PropagationRecord propagationRecord = this.propagateionDataFactory.buildPropagationRecord();
				RuleCommentAnnotation ruleCommentAnnotation = this.propagateionDataFactory.buildRuleCommentAnnotation(rule.getRuleAC(),
						rule.getTemplateAC(), null);
				EntryCommentAnnotation entryCommentAnnotation = this.propagateionDataFactory.buildEntryCommentAnnotation(entryAC, entryID,
	...
	private boolean ruleHasComment(PIRRule rule, EntryComment comment) {
		for (Line line : rule.getAnnotationSection().getLines()) {
			if (line instanceof CCLine) {
				...
				if (ccLine.getTopic().name().equals(comment.getCommentType().name())) {
					return true;
				}
			}
		}
	...
	private PropagationRecord propagateFeatureDescriptionLine(Alignment alignment, UniProtEntry entry, PIRRule rule, Line ruleLine,
			ControlStatement controlStatement, ExpressionValue controlStatementEval, String ftGroupMatchNote, Map<Integer, Boolean> ftGroups) {
		PropagationRecord propagationRecord = this.propagateionDataFactory.buildPropagationRecord();
		...
		String note = "";
		RuleFeatureTableAnnotation ruleFeatureTableAnnotation = this.propagateionDataFactory.buildRuleFeatureTableAnnotation(ruleAC, templateAC,
				(FeatureDescriptionLine) ruleLine);
		EntryFeatureTableAnnotation entryFeatureTableAnnotation = this.propagateionDataFactory.buildEntryFeatureTableAnnotation(entryAC, entryID,
				entryType, null, "");
		if (ftGroupOK(ftGroups)) {
			if (controlStatementEval.equals(ExpressionValue.TRUE)) {
				entryFeatureTableAnnotation = checkMatchedEntryFeature(alignment, entry, ruleFeatureTableAnnotation);
				if (entryFeatureTableAnnotation != null) {
					if (entryFeatureTableAnnotation.getMissmatchReason().equals("")) {
...
//						} else {
						...
						note += PropagationUtil.getMatchInfo(entry, alignment, ruleFeatureTableAnnotation.getFeatureDescriptionLine()) + "; ";
					...
					if (PropagationUtil.checkFeaturePatternMatch(entry, alignment, ruleFeatureTableAnnotation.getFeatureDescriptionLine())) {
						EntryFeatureTable entryFeature = getEntryFeature(alignment, ruleFeatureTableAnnotation);

						entryFeatureTableAnnotation = this.propagateionDataFactory.buildEntryFeatureTableAnnotation(entryAC, entryID, entryType,
								entryFeature, "");

						note += "Predicated feature<-|->" + entryFeatureTableAnnotation.getMissmatchReason();
						note += PropagationUtil.getMatchInfo(entry, alignment, ruleFeatureTableAnnotation.getFeatureDescriptionLine()) + "; ";
						...
						if (note.contains("Template start or end residue does not match")) {
							...
							if (!note.contains("Residue pattern does not match<-|->")) {
								propagationRecord.setPropagationStatistics(PropagationStats.FP);
								propagationRecord.setPropagated(true);
								propagationRecord.setMatched(false);
								propagationRecord.setNote(note);
						...
						EntryFeatureTable entryFeature = factory.buildEntryFeatureTable(
								EntryFeatureType.valueOf(ruleFeatureTableAnnotation.getFeatureType().name()), "", "");
						entryFeature.setFeatureDescription("");
						entryFeatureTableAnnotation = this.propagateionDataFactory.buildEntryFeatureTableAnnotation(entryAC, entryID, entryType,
								entryFeature, "");

						note += "No matched entry feature found<-|->" + entryFeatureTableAnnotation.getMissmatchReason();
						note += PropagationUtil.getMatchInfo(entry, alignment, ruleFeatureTableAnnotation.getFeatureDescriptionLine()) + "; ";
						propagationRecord.setPropagated(false);
						propagationRecord.setMatched(false);
						propagationRecord.setPropagationStatistics(PropagationStats.NA);
				...
				propagationRecord.setMatched(false);
				propagationRecord.setPropagationStatistics(PropagationStats.NA);
				note += "Control statement evaluted to be false, no matched feature<-|->" + ftGroupMatchNote;
			}
...
//		return true;
	...
	private EntryFeatureTable getEntryFeature(Alignment alignment, RuleFeatureTableAnnotation ruleFeatureTableAnnotation) {
		EntryFeatureTable feature = null;

		FeatureDescriptionLine featureDescriptionLine = ruleFeatureTableAnnotation.getFeatureDescriptionLine();
		String ruleFeatureDescription = "";

		for (int i = 0; i < featureDescriptionLine.getDescriptions().size(); i++) {

			if (i == 0) {
				ruleFeatureDescription = featureDescriptionLine.getDescriptions().get(i).trim();
			} else {
				ruleFeatureDescription += " " + featureDescriptionLine.getDescriptions().get(i).trim();
			}
		}
		ruleFeatureDescription = ruleFeatureDescription.trim();
		String ruleFeatureStart = ruleFeatureTableAnnotation.getFeatureDescriptionLine().getFeatureFromPosition();
		String ruleFeatureEnd = ruleFeatureTableAnnotation.getFeatureDescriptionLine().getFeatureToPosition();
		String entryFeatureStart = PropagationUtil.convertTemplatePositionToTargetPosition(alignment, ruleFeatureStart);
		String entryFeatureEnd = PropagationUtil.convertTemplatePositionToTargetPosition(alignment, ruleFeatureEnd);

		feature = factory.buildEntryFeatureTable(EntryFeatureType.valueOf(ruleFeatureTableAnnotation.getFeatureType().name()), entryFeatureStart,
				entryFeatureEnd);
	...
	private PropagationRecord propagateCCLine(Alignment alignment, UniProtEntry entry, PIRRule rule, Line ruleLine,
			ControlStatement controlStatement, ExpressionValue controlStatementEval, FTGroupMatch ftGroupMatchResult) {
		PropagationRecord propagationRecord = this.propagateionDataFactory.buildPropagationRecord();
		propagationRecord.setControlStatement(controlStatement);
		propagationRecord.setControlStatementEvalution(controlStatementEval);
		...
		String note = "";
		ruleCommentAnnotation = this.propagateionDataFactory.buildRuleCommentAnnotation(ruleAC, templateAC, (CCLine) ruleLine);
		entryCommentAnnotation = this.propagateionDataFactory.buildEntryCommentAnnotation(entryAC, entryID, entryType, null);

		if (controlStatementEval.equals(ExpressionValue.TRUE)) {
			entryCommentAnnotation = checkMatchedEntryComment(entry, ruleCommentAnnotation);
...
//				} else {
				...
				EntryComment comment = getEntryComment(ruleCommentAnnotation);

				// comment.setCommentDescription("");
				// System.out.println(comment.get);
				entryCommentAnnotation = this.propagateionDataFactory.buildEntryCommentAnnotation(entryAC, entryID, entryType, comment);
				// System.out.println(entryCommentAnnotation);
				propagationRecord.setMatched(false);
//				if (isSwissProt) {
//					// propagationRecord.setPropagated(false);
//					propagationRecord.setPropagated(true);
//					note += "Comment mismatch<-|->Predicated comment<-|->" + "Rule: " + ruleCommentAnnotation.getCommentType() + " ["
//							+ ruleCommentAnnotation.getCommentDescription() + "]<-|->Entry: "
//							+ this.getEntryCommentByType(entry, ruleCommentAnnotation.getCommentType().name());
//					propagationRecord.setPropagationStatistics(PropagationStats.FP);
//				} else {
			...
			propagationRecord.setPropagationStatistics(PropagationStats.NA);
			note += "Control statement evaluted to be false, no match comment<-|->" + ftGroupMatchResult.getNote();
		}
	...
	private EntryComment getEntryComment(RuleCommentAnnotation ruleCommentAnnotation) {
		EntryCommentType commentType = EntryCommentType.valueOf(ruleCommentAnnotation.getCommentType().name());
		//String commentDescription = ruleCommentAnnotation.getCommentDescription();

		EntryComment comment = factory.buildEntryComment(commentType, ruleCommentAnnotation.getComment().getDescription());

		return comment;

	}

	private EntryFeatureTableAnnotation checkMatchedEntryFeature(Alignment alignment, UniProtEntry entry,
		...
		String ruleFeatureDescription = ruleFeatureTableAnnotation.getDescription();
		String ruleFeatureType = ruleFeatureTableAnnotation.getFeatureType().name();

		List<EntryFeatureTable> featureList = entry.getFeatures();
		for (EntryFeatureTable feature : featureList) {
			entryFeatureTableAnnotation = this.propagateionDataFactory.buildEntryFeatureTableAnnotation(entryAC, entryID, entryType, feature, "");

			String entryFeatureDescription = entryFeatureTableAnnotation.getFeatureDescription();
			String entryFeatureType = feature.getFeatureType().name();

			if (ruleFeatureType.equals(entryFeatureType) && feature.getFromEndPoint().endsWith(entryFeatureStart)
					&& feature.getToEndPoint().endsWith(entryFeatureEnd)) {
				if (ruleFeatureDescription.replaceAll("\\.$", "").equals(entryFeatureDescription.replaceAll("\\.$", ""))) {
					// System.out.println(entryFeatureTableAnnotation.getFeature()+"!!!");
					return this.propagateionDataFactory.buildEntryFeatureTableAnnotation(entryAC, entryID, entryType, feature, "");
				}
			}
		}
		return null;
	...
	private EntryCommentAnnotation checkMatchedEntryComment(UniProtEntry entry, RuleCommentAnnotation ruleCommentAnnotation) {
		EntryCommentAnnotation entryCommentAnnotation = null;
		String entryAC = entry.getPrimiaryAccessionNumber();
		String entryID = entry.getEntryId();
		EntryType entryType = entry.getEntryType();

		String ruleComment = ruleCommentAnnotation.getCommentType() + " | " + ruleCommentAnnotation.getCommentDescription();
		List<EntryComment> entryComments = entry.getComments();
		for (EntryComment comment : entryComments) {
			entryCommentAnnotation = this.propagateionDataFactory.buildEntryCommentAnnotation(entryAC, entryID, entryType, comment);
			String entryComment = comment.getCommentType() + " | " + entryCommentAnnotation.getCommentDescription();
			// System.out.println("???");
			// System.out.println(ruleComment);
			// System.out.println(entry.getPrimiaryAccessionNumber()
			// +"\n"+entryComment);
			// System.out.println("\n");
			if (ruleComment.replaceAll("\\.$", "").equals(entryComment.replaceAll("\\.$", ""))) {
	...
	private PropagationRecord propagateKWLine(Alignment alignment, UniProtEntry entry, PIRRule rule, Line ruleLine,
			ControlStatement controlStatement, ExpressionValue controlStatementEval, FTGroupMatch ftGroupMatchResult) {
		PropagationRecord propagationRecord = this.propagateionDataFactory.buildPropagationRecord();
		propagationRecord.setControlStatement(controlStatement);
		propagationRecord.setControlStatementEvalution(controlStatementEval);
		...
		String note = "";
		ruleKeywordAnnotation = this.propagateionDataFactory.buildRuleKeywordAnnotation(ruleAC, templateAC, (KWLine) ruleLine);
		entryKeywordAnnotation = this.propagateionDataFactory.buildEntryKeywordAnnotation(entryAC, entryID, entryType, null);
		// System.out.println(entryKeywordAnnotation.getEntryAC() + " | " +
		// entryKeywordAnnotation.isSwissProt());
		if (controlStatementEval.equals(ExpressionValue.TRUE)) {
			entryKeywordAnnotation = checkMatchedEntryKeyword(entry, (KWLine) ruleLine);
...
//				} else {
				...
				EntryKeyword keyword = factory.buildEntryKeyword(ruleKeywordAnnotation.getKeyword().getKeyword());

				entryKeywordAnnotation = this.propagateionDataFactory.buildEntryKeywordAnnotation(entryAC, entryID, entryType, keyword);
...
//				} else {
			...
			note += "Control statement evaluted to be false, no matched keyword<-|->" + ftGroupMatchResult.getNote();
		}
	...
	private EntryKeywordAnnotation checkMatchedEntryKeyword(UniProtEntry entry, KWLine ruleLine) {
		EntryKeywordAnnotation entryKeywordAnnotation = null;
		...
		for (EntryKeyword entryKeyword : keywordList) {
			if (entryKeyword.getKeyword().equals(ruleKeyword)) {
				entryKeywordAnnotation = this.propagateionDataFactory.buildEntryKeywordAnnotation(entryAC, entryID, entryType, entryKeyword);
				break;
			}
		}
		// if(entryKeywordAnnotation == null) {
		...
		for (int i = 0; i < listOfFiles.length; i++) {
			if (listOfFiles[i].isFile()) {
				...
				if (fileName.endsWith(".uru")) {
					...
					ruleFilePath = propagationDirectory + System.getProperty("file.separator") + fileName;
				...
				if (fileName.endsWith(".dat")) {
					...
					dataFilePath = propagationDirectory + System.getProperty("file.separator") + fileName;
					File file = new File(dataFilePath);
					if (file.length() == 0) {
						checkFilesErrors += "'" + dataFilePath + "' is empty.\n";
					}
				}
				if (fileName.endsWith(".gff")) {
					...
					alignmentFilePath = propagationDirectory + System.getProperty("file.separator") + fileName;
					File file = new File(alignmentFilePath);
					if (file.length() == 0) {
						checkFilesErrors += "'" + alignmentFilePath + "' is empty.\n";
					}
				}
			}
		}

...
import edu.udel.bioinformatics.pirsr.ScopeKingdom;
...
public class UniRuleFlatFileParser {
	...
	public UniRuleFlatFileParser(PIRRuleManager pirruleManager, boolean isStrict, BufferedReader br) {
		super();
		...
		while ((line = br.readLine()) != null) {
			...
			if (line.length() > 0) {
				...
				if (line.startsWith(UniRuleFlatFileConstants.AC_LINE_START)) {
					...
					if (!line.startsWith(UniRuleFlatFileConstants.HISTORY_FIRST_CREATE)
							&& !line.startsWith(UniRuleFlatFileConstants.HISTORY_LAST_MODIFIED)) {
	...
	private void parseScopeFusionBlockContinousLine(String line) {
		if (line.startsWith(UniRuleFlatFileConstants.FUSION_NTER_START)) {
			...
			if (!nTerDescription.equals("None")) {
				...
				if (controlBlock != null) {
					if (controlBlock instanceof ControlBlock) {
						ControlStatement lastControlStatement = controlBlock.getControlStatements().get(
								controlBlock.getControlStatements().size() - 1);
	...
	private DefinedOperatorExpression parseDefinedOperatorExpression(String line) {
		...
		if (this.isBalanced(line) && line.startsWith(UniRuleFlatFileConstants.LEFT_PARENTHESIS)
				&& line.endsWith(UniRuleFlatFileConstants.RIGHT_PARENTHESIS)) {
	...
	private ConditionExpression parseOrganismConditionExpression(String line) {
		...
		if (this.isBalanced(line) && line.startsWith(UniRuleFlatFileConstants.LEFT_ANGLE_BRAKET)
				&& line.endsWith(UniRuleFlatFileConstants.RIGHT_ANGLE_BRAKET)) {
			line = line.substring(1, line.length() - 1);
			line = line.trim();
		}
	...
	private ConditionExpression parseFTGroupConditionExpression(String line) {
		...
		FTGroupConditionExpression ftGroupConditionExpression = this.dataFactory
				.buildFeatureConditionExpression(ftGroupNumber, ExpressionValue.FALSE);
...
//				errorHandler(errorMessage);
//			} else {
//				String message = "Invalid DE line, missing ';' at the end of line.";
//				ErrorMessage errorMessage = new ErrorMessage(currentRuleLineCount, currentLineCount, message);
//				errorHandler(errorMessage);
//			}
		}
	...
	private void parseFTLine(String line) {
			...
			if (controlBlock instanceof ControlBlock) {
				...
				if (lastControlStatement.getApplicableLines() == null) {
					...
					if (!(lastLine instanceof FeatureDescriptionLine)) {
						String message = " [Line: " + currentRuleLineCount + " (" + currentLineCount + ")]: "
								+ " Missing FT feature description line.";
						ErrorMessage errorMessage = new ErrorMessage(currentRuleLineCount, currentLineCount, message);
						errorHandler(errorMessage);
					} else {
		...
		// FT constraint line
			...
			String conditionDesc = featureConstraintLineMatcher.group(3);

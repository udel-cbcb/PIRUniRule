...
*/
	...
	public static String getTemplateResidues(Alignment alignment, String start, String end) {
		if(debug) {
			System.out.println("enter getTemplateResidues: start: "+start+ " end: "+end);
		}
		int templateResiduesStart = -1;
		int templateResiduesEnd = -1;
		...
		if(debug) {
			System.out.println("templateResiduesStart: "+ templateResiduesStart + " | templateResiduesEnd: "+templateResiduesEnd);
		}
		String residues = "";
		char[] templateSeqChars = alignment.getAlignedTemplateSeq().toCharArray();
		...
		if (debug) {
			System.out.println("Template residues: " + start + " | " + end + " | " + residues + " | ");
		}
		return residues;
	}
...
//		String alignedSeqEnd = convertTemplatePositionToTargetPosition(alignment, featureDescriptionLine.getFeatureToPosition());
		String alignedSeqStart ="";
		String alignedSeqEnd = "";
		if(featureDescriptionLine.getFeatureFromPosition().equals("Nter") || featureDescriptionLine.getFeatureFromPosition().equals("?")) {
			alignedSeqStart = convertTemplatePositionToTargetPosition(alignment, featureDescriptionLine.getFeatureToPosition());
			if(debug) {
				System.out.println("I am here From: "+ alignedSeqStart);
			}
		}
		else {
			alignedSeqStart = convertTemplatePositionToTargetPosition(alignment, featureDescriptionLine.getFeatureFromPosition());
		}
		if(featureDescriptionLine.getFeatureToPosition().equals("Cter") || featureDescriptionLine.getFeatureToPosition().equals("?")) {
			alignedSeqEnd = convertTemplatePositionToTargetPosition(alignment, featureDescriptionLine.getFeatureFromPosition());
			if(debug) {
				System.out.println("I am here To: " + alignedSeqEnd);
			}
		}
		else {
			alignedSeqEnd = convertTemplatePositionToTargetPosition(alignment, featureDescriptionLine.getFeatureToPosition());
		}
		if(debug) {
			System.out.println("FDesc postions: "+ featureDescriptionLine.getFeatureFromPosition()+ " | "+ featureDescriptionLine.getFeatureToPosition());
		}
		String conditionPattern = featureDescriptionLine.getFeatureConditionPattern();
		//System.out.println(featureDescriptionLine.getFeatureKey().toString()+"|"+conditionPattern+"|");
		if (featureDescriptionLine.getFeatureKey().toString().equals("CARBOHYD") && (conditionPattern.equals("N-x-T") || conditionPattern.equals("N-x-S")||conditionPattern.equals("N-x-[T]") || conditionPattern.equals("N-x-[S]") ||conditionPattern.equals("N-x-[ST]")||conditionPattern.equals("N-x-[S/T]")||conditionPattern.equals("N-x-[TS]")||conditionPattern.equals("N-x-[T/S]"))) {
		//if (featureDescriptionLine.getFeatureKey().toString().equals("CARBOHYD") && (conditionPattern.matches("N-x-.{1,5}"))) {
			...
			if(debug) {
				System.out.println("???? alignedSeqStart " + alignedSeqStart);
				System.out.println("???? adjustedAlignedSeqEnd " + adjustedAlignedSeqEnd);
				System.out.println("???? alignedSeqResidues "+alignedSeqResidues);
			}
			boolean patternMatch = checkPatternMatch(alignedSeqResidues, conditionPattern);
			if(debug) {
				System.out.println("???? patternmatch "+patternMatch);
				System.out.println("Enter getFeatureDescriptionMatch adjusted end: "+adjustedAlignedSeqEnd);
			}
			...
			note += "Template (FTGroup " + ftGroupNumber + "): " + featureDescriptionLine.getFeatureFromPosition() + "| "
					...
					+ getTemplateResidues(alignment, featureDescriptionLine.getFeatureFromPosition(), String.valueOf(Integer.parseInt(featureDescriptionLine.getFeatureToPosition())+2)) + " | "
					+ alignment.getTemplateEntryAC() + " | " + featureDescriptionLine.getFeatureConditionPattern() + "<-|->";
			note += "Target: " + alignedSeqStart + "| " + adjustedAlignedSeqEnd + " | " + alignedSeqResidues + " | " + entry.getPrimiaryAccessionNumber() + " | "
	...
	public static boolean checkFeaturePatternMatch(UniProtEntry entry, Alignment alignment, FeatureDescriptionLine featureDescriptionLine) {
		if(debug) {
			System.out.println("checkFeaturePatternMatch: entry: "+entry.getPrimiaryAccessionNumber() + " FTDesc: "+ featureDescriptionLine);
		}
//		String alignedSeqStart = convertTemplatePositionToTargetPosition(alignment, featureDescriptionLine.getFeatureFromPosition());
//		String alignedSeqEnd = convertTemplatePositionToTargetPosition(alignment, featureDescriptionLine.getFeatureToPosition());
		String alignedSeqStart ="";
		...
		if (featureDescriptionLine.getFeatureKey().toString().equals("CARBOHYD") && (conditionPattern.equals("N-x-T") || conditionPattern.equals("N-x-S")||conditionPattern.equals("N-x-[T]") || conditionPattern.equals("N-x-[S]") ||conditionPattern.equals("N-x-[ST]")||conditionPattern.equals("N-x-[S/T]")||conditionPattern.equals("N-x-[TS]")||conditionPattern.equals("N-x-[T/S]"))) {
			...
			//System.out.println("!!!" + alignedSeqResidues);

			if (debug) {
				System.out.println("enter checkFeaturePatternMatch: "+featureDescriptionLine.getFeatureFromPosition() + "| " + featureDescriptionLine.getFeatureToPosition() + " | "
						//+ getTemplateResidues(alignment, featureDescriptionLine.getFeatureFromPosition(), featureDescriptionLine.getFeatureToPosition())
						+ getTemplateResidues(alignment, featureDescriptionLine.getFeatureFromPosition(), String.valueOf(Integer.parseInt(featureDescriptionLine.getFeatureToPosition())+2))
						+ " | " + alignment.getTemplateEntryAC() + " | " + featureDescriptionLine.getFeatureConditionPattern());
			...
			if(debug) {
				System.out.println("getAlignedEntryResidue: " + alignedSeqStart + " | " + alignedSeqEnd);
			}
			alignedSeqResidues = getAlignedEntryResidues(alignment, alignedSeqStart, alignedSeqEnd);

			if (debug) {
				System.out.println("enter checkFeaturePatternMatch 2: "+featureDescriptionLine.getFeatureFromPosition() + "| " + featureDescriptionLine.getFeatureToPosition() + " | "
						+ getTemplateResidues(alignment, featureDescriptionLine.getFeatureFromPosition(), featureDescriptionLine.getFeatureToPosition())
						+ " | " + alignment.getTemplateEntryAC() + " | " + featureDescriptionLine.getFeatureConditionPattern());
				System.out.println(alignedSeqStart + "| " + alignedSeqEnd + " | " + alignedSeqResidues + " | " + entry.getPrimiaryAccessionNumber());
	...
	public static Boolean checkPatternMatch(String alignedSeqResidues, String conditionPattern) {
		...
		if (debug) {
			System.out.println("match pattern: " +matchPattern + " | " + alignedSeqResidues + " | " + alignedSeqResidues.matches(matchPattern));
			System.out.println();
		}
	...
	public static FTGroupMatch getFTGroupMatch(UniProtEntry entry, Alignment alignment, PIRRule rule) {
			...
			for (ControlStatement controlStatement : controlStatements) {
				...
				for (Line line : applicableLines) {
					if (line instanceof FeatureDescriptionLine) {
						...
						//System.out.println(featureDescriptionLine);
		...
		if(debug) {
			System.out.println(rule.getRuleAC() + " ftGroupMatches: " +ftGroupMatches);
		}
	...
	public static String getMatchInfo(UniProtEntry entry, Alignment alignment, FeatureDescriptionLine featureDescriptionLine) {
		if(debug) {
			System.out.println("enter getMatchInfo: entry: "+entry.getPrimiaryAccessionNumber() + " FTDesc: "+ featureDescriptionLine);
		}
		//		String alignedSeqStart = convertTemplatePositionToTargetPosition(alignment, featureDescriptionLine.getFeatureFromPosition());
//		String alignedSeqEnd = convertTemplatePositionToTargetPosition(alignment, featureDescriptionLine.getFeatureToPosition());
		String alignedSeqStart ="";
		...
		if (featureDescriptionLine.getFeatureKey().toString().equals("CARBOHYD") && (conditionPattern.equals("N-x-T") || conditionPattern.equals("N-x-S")||conditionPattern.equals("N-x-[T]") || conditionPattern.equals("N-x-[S]") ||conditionPattern.equals("N-x-[ST]")||conditionPattern.equals("N-x-[S/T]")||conditionPattern.equals("N-x-[TS]")||conditionPattern.equals("N-x-[T/S]"))) {
			...
			if (!getFeatureType(entry, alignedSeqStart, alignedSeqEnd, featureDescriptionLine.getFeatureKey()).equals("")) {
				if (!checkPatternMatch(alignedSeqResidues, conditionPattern)) {
					...
					if(debug) {
						System.out.println("pattern mismatch: " + alignedSeqResidues + " | "+ conditionPattern);
					}
				} else {
					if (!getFeatureType(entry, alignedSeqStart, alignedSeqEnd, featureDescriptionLine.getFeatureKey()).equals(

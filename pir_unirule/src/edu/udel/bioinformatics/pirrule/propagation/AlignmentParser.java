...
public class AlignmentParser {
	...
	public AlignmentParser(PropagationManager annotationManager,
			BufferedReader br) {
		...
		try {
			while (br.ready()) {
				...
				if (s.indexOf("SRHMM") != -1) {
					...
					String seqAln = sequence.replaceFirst("Sequence \"", "")
							.replaceAll("\"", "");

					String[] featureTemplate = template.split(" ");
					String templateEntryAC = featureTemplate[1].replaceAll(
							"\"", "");
					String templateAln = featureTemplate[2]
							.replaceAll("\"", "");
					Alignment aln = this.annotationManager
							.getPropagationDataFactory().buildAlignment();
					...
					if (debug) {
						if (count % 10000 == 0) {
							System.out.println("Load alignment: " + count);
						}
					}

					// System.out.println(aln.getEntryId() + "\t"
					// +aln.getRuleAC()+"\t" + aln.getTrigger() +
					// "\t"+aln.getTemplateEntryAC());
				}

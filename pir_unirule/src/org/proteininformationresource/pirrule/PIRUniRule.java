package org.proteininformationresource.pirrule;
...
import edu.udel.bioinformatics.pirrule.PIRRuleManagerImpl;
import edu.udel.bioinformatics.pirrule.profilehmm.ProfileHMMManagerImpl;
...
public class PIRUniRule {
	...
	public static void main(String[] args) {
		...
		Option actionOpt = OptionBuilder.withArgName("arg").hasArg(true).withLongOpt("action").isRequired()
		...
		options.addOption("D", "debug", false, "Debug mode, print out rule line by line");
		...
		try {
			...
			if (line.getOptions().length == 0 || line.hasOption("h")) {
				// printHelp(int width, String cmdLineSyntax, String header,
				...
				if (!hasCommandLineError) {
					if (action.equals("checkRuleSyntax")) {
						...
						if (ruleFile == null) {
							...
							//PropagationManager propagationManager = propagationDataFactory.buildPropagationManager(propagationDataFactory, pirruleManager);
							...
							if (lineNumber != null && lineNumber.toUpperCase().equals("SOURCE")) {
								rules = pirruleManager.parsePIRRules(new File(ruleFile), PIRRuleManager.Format.UNIRULE, false, debug, "Source");

								writer.print(pirruleManager.getParserLogs());
							} else if (lineNumber != null && lineNumber.toUpperCase().equals("RULE")) {
								rules = pirruleManager.parsePIRRules(new File(ruleFile), PIRRuleManager.Format.UNIRULE, false, debug, "Rule");
								writer.print(pirruleManager.getParserLogs());
							} else {
								// System.out.println(" I am here");
								rules = pirruleManager.parsePIRRules(new File(ruleFile), PIRRuleManager.Format.UNIRULE, false, debug, "");
								writer.print(pirruleManager.getParserLogs());
							}
						}
						...
						if (!hasCommandLineError) {
							...
							List<PIRRule> rules = pirruleManager.parsePIRRules(new File(ruleFile), PIRRuleManager.Format.UNIRULE, false, debug, "");
	...
	private static void checkHMM3Model(PIRRule rule, String templateToModelSearchOutputFile, PrintWriter writer) {
		...
		for (Line line : rule.getAnnotationSection().getLines()) {
			...
			if (line instanceof FeatureDescriptionLine) {
				...
				String from = featureDescriptionLine.getFeatureFromPosition();
				if (from.equals("Nter")) {
					from = featureDescriptionLine.getFeatureToPosition();
				}

				String to = featureDescriptionLine.getFeatureToPosition();
				if (to.equals("Cter")) {
					to = featureDescriptionLine.getFeatureFromPosition();
				}
				String matchState = "";
				// if (from.equals(to)) {
				// matchState =
				// checkTemplateProfileMatchState(Integer.parseInt(from),
				// template);
				// } else {
				// for (int i = Integer.parseInt(from); i <=
				...
				// template);
				// matchState +=
				// checkTemplateProfileMatchState(Integer.parseInt(to),
				// template);
		...
		for (PIRRule rule : rules) {
			...
			for (UniProtEntry entry : swissProtEntries) {
				if (entry.getPrimiaryAccessionNumber().equals(templateAC)) {
					...
					if (rule.getTrigger().startsWith("PIRSF")) {
						if(!rule.getTrigger().startsWith("PIRSR6")) {
							if (entry.getDatabaseCrossReferences(EntryDatabaseType.PIRSF) == null) {
								...
								for (EntryDatabaseCrossReference dbRef : entry.getDatabaseCrossReferences(EntryDatabaseType.PIRSF)) {
									if (dbRef.getIdentifier().equals(rule.getTrigger())) {
										triggerMatch = true;
									}
								}
								if (!triggerMatch) {
							...
							if (entry.getDatabaseCrossReferences(EntryDatabaseType.InterPro) == null) {
								note += "Template \"" + entry.getPrimiaryAccessionNumber() + "\" has no InterPro DR line.\n";
							} else {
								boolean triggerMatch = false;
								for (EntryDatabaseCrossReference dbRef : entry.getDatabaseCrossReferences(EntryDatabaseType.InterPro)) {
									if (dbRef.getIdentifier().equals(rule.getTrigger())) {
										triggerMatch = true;
									}
								}
					...
					} else if (rule.getTrigger().startsWith("IPR")) {
						if (entry.getDatabaseCrossReferences(EntryDatabaseType.InterPro) == null) {
							note += "Template \"" + entry.getPrimiaryAccessionNumber() + "\" has no InterPro DR line.\n";
						} else {
							boolean triggerMatch = false;
			...
			if (note.equals("")) {
				...
				writer.println("Rule " + ruleAC + ", Template " + templateAC + ", " + rule.getLastModifyCurator() + " (" + rule.getLastModifyDateTime()
						+ "): \n\n" + note + "\n");
			}
			// writer.println("\n");
		}
	...
	private static String checkFeatureResidue(FeatureDescriptionLine ruleLine, List<EntryFeatureTable> entryFeatures, UniProtEntry entry, PIRRule rule) {
		...
		for (EntryFeatureTable entryFeature : entryFeatures) {
			...
			if (ruleLine.getFeatureFromPosition().equals("Nter")) {
				ruleStart = ruleLine.getFeatureToPosition();
			} else {
				ruleStart = ruleLine.getFeatureFromPosition();
			}
			String ruleEnd = "";
			if (ruleLine.getFeatureToPosition().equals("Cter")) {
				ruleEnd = ruleLine.getFeatureFromPosition();
			} else {
				ruleEnd = ruleLine.getFeatureToPosition();
			}

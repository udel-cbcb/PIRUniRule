...
_Bel = 0;
_scrmt = setTimeout("", 0);
mac = (_nu.indexOf("macintosh") != -1) ? _t : _f;
_amt = "";
_MTF = 0;
_onTS = 0;
var _cel = -1;
function _sLayer(_obj, _show) {
	if (_obj.visibility != _show)
		_obj.visibility = _show
}
function $CtI($ti) {
	clearTimeout($ti);
	$ti = _n
...
function drawItem(_i) {
	...
	_iME = "";
	if (_lnk) {
		_iMS = "<a href=\"" + _lnk + "\" onMouseOver=\"set_status(" + _i
				+ ");return true\">";
		_iME = "</a>"
	}
	_Lsimg = "";
	...
	if (_mi[_i][3] && _mi[_i][24]) {
		...
		_its = _itrs + "<td " + _imps + "><table border=0 cellspacing="
				+ _simgP + " cellpadding=0><td>";
	...
	if (_mi[_i][29]) {
		...
		_img = _iMS + "<img " + _Iwid + _Ihgt + " border=0 src=" + _mi[_i][29]
				+ ">" + _iME;
		_oimg = _img;
		if (_mi[_i][32])
			_oimg = _iMS + "<img " + _Iwid + _Ihgt + " border=0 src="
					+ _mi[_i][32] + ">" + _iME;
		if (!_mi[_i][30])
			_mi[_i][30] = "left";
	...
	if (_mi[_i][1])
		_txt = _mi[_i][1];
	_acT = "onmouseover=\"_popi(" + _i
			+ ");clearTimeout(_MTF);_MTF=setTimeout('close_el(" + _i
			+ ")',200);\";drag_drop('menu" + _Dmnu + "');";
	...
	_Tmt += "<div></div>";
	_Tmt += "<table " + _wid + " " + _bgc
			+ " border=0 cellpadding=0 cellspacing=0 width=100%>";
	_Tmt += _Limg;
	_Tmt += _Lsimg;
	if (_txt) {
		_Tmt += "<td width=100%><table " + _hgt + " border=0 cellpadding="
				+ _pad + " cellspacing=0 width=100%><td " + _algn + _nw + " >";
		_Tmt += "<a href=\"\" class=item" + _i + " onMouseOver=\"set_status("
				+ _i + ");return true\">";
		_Tmt += _fgc + _txt;
		_Tmt += "</a>";
		_Tmt += "</td></table></td>"
	...
	_Tmt += "</table>";
	_Tmt += "</layer>";
	_Tmt += "<layer visibility=hide id=oel"
			+ _i
			+ " zindex=999 onMouseOver=\"clearTimeout(_MTF);_back2par("
			+ _i
			+ ");nshl="
			+ _i
			+ ";this.captureEvents(Event.MOUSEUP);this.onMouseUp=_lc;\" onMouseOut=\"close_el("
			+ _i + ")\" width=100%>";
	_Tmt += "<div></div>";
	_Tmt += "<table " + _wid + " " + _bgbc
			+ " border=0 cellpadding=0 cellspacing=0 width=100%>";
	_Tmt += _LimgO;
	_Tmt += _LsimgO;
	if (_txt) {
		_Tmt += "<td height=1 width=100%><table " + _hgt
				+ " border=0 cellpadding=" + _pad
				+ " cellspacing=0 width=100%><td " + _algn + _nw + " >";
		_Tmt += "<a class=oitem" + _i + " href=\"" + _lnk
				+ "\" onMouseOver=\"set_status(" + _i + ");return true\">";
		_Tmt += _fgbc + _txt;
		_Tmt += "</a>";
		_Tmt += "</td></table></td>"
	}
	...
	if (!_M[9]) {
		...
		if ((_i != _M[0][_M[0].length - 1]) && _mi[_i][27] > 0) {
			...
			if (_mi[_i][16] && _mi[_i][17]) {
				...
				if (_bwid < 1)
					_bwid = 1;
				_Tmt += "<td bgcolor=" + _mi[_i][17] + "><spacer type=block "
						+ _hgt + " width=" + _bwid + "></td>";
				_Tmt += "<td bgcolor=" + _mi[_i][16] + "><spacer type=block "
						+ _hgt + " width=" + _bwid + "></td>"
			} else {
				if (_spd)
					_Tmt += "<td><spacer type=block width=" + _spd + "></td>";
				_Tmt += "<td " + _sbg + " bgcolor=" + _mi[_i][10]
						+ "><spacer type=block " + _hgt + " width="
						+ _mi[_i][27] + "></td>";
				if (_spd)
					_Tmt += "<td><spacer type=block width=" + _spd + "></td>"
			}
...
function _drawMenu(_mnu) {
	...
	if (_M[9]) {
		_oldBel = _Bel;
		_d.write("<layer visibility=hide id=HT" + _mnu
	...
	_bImg = "";
	if (_ms.menubgimage)
		_bImg = " background=" + _ms.menubgimage;
	if (_M[6][46])
		_bImg = "background=" + _M[6][46];
	if (_M[14] != "relative")
		_mt += "<layer zindex=999 "
				+ _bImg
				+ " onmouseout=\"close_menu()\" onmouseover=\"clearTimeout(_MT);\" id=menu"
				+ _mnu + " " + _top + " " + _left + " " + _visi + ">";
	_bgc = "";
	...
	if (_M[12])
		_mrg = _M[12];
	_mt += "<table " + _bgc + " border=0 cellpadding=" + _mrg
			+ " cellspacing=0 >";
	_mt += "<td>";
	...
	if (_M[14] != "relative") {
		...
		_d.write("<style>" + _st + "</style>");
		_gm = gmobj("menu" + _mnu);
		_d.write("<layer visibility=hide id=bord" + _mnu
				+ " zindex=0 class=menu" + _mnu + "><spacer width="
				+ (_gm.clip.width - 6) + " type=block height="
				+ (_gm.clip.height - 6) + "></layer>");
		if (_M[7]) {
			_gm = gmobj("menu" + _mnu);
			_gm.zIndex = 999;
			_gp = gpos(_gm);
	...
	if (_m[_mnu][13] == "scroll") {
		...
		if (_gm) {
			...
			_scs = ";this.bgColor='" + _m[_mnu][6].onbgcolor
					+ "'\" onmouseout=\"csto(" + _mnu + ");this.bgColor='"
					+ _m[_mnu][6].offbgcolor + "'\"";
			_scs += " visibility=hide " + _bgc + " class=menu" + _mnu
					+ "><table border=0 cellpadding=0 cellspacing=0 width="
					+ (_gm.clip.width - 6) + "><td align=center>";
			_sce = "</td></table></layer>";
			_upSImage = "<<";
			_downSImage = ">>";
			if (window._scrollUpImage)
				_upSImage = "<img src=" + window._scrollUpImage + ">";
			if (window._scrollDownImage)
				_downSImage = "<img src=" + window._scrollDownImage + ">";
			if (!window._scrollAmount)
				_scrollAmount = 5;
			_d.write("<layer id=tscroll" + _mnu + " onmouseover=\"_is(" + _mnu
					+ "," + _scrollAmount + ");" + _scs + _upSImage + _sce);
			_d.write("<layer id=bscroll" + _mnu + " onmouseover=\"_is(" + _mnu
					+ ",-" + _scrollAmount + ");" + _scs + _downSImage + _sce);
...
function clearELs(_i) {
	...
	for (_q = 0; _q < _m[_mnu][0].length; _q++) {
		_sLayer(gmobj("oel" + _m[_mnu][0][_q]), "hide")
	}
}
function menuDisplay(_mnu, _show) {
	_gm = gmobj("menu" + _mnu);
	_gmb = gmobj("bord" + _mnu);
	if (_gm.visibility == _show)
		return;
	...
	if (_show) {
		...
		if (_m[_mnu][13] == "scroll") {
			...
			if (horiz) {
				if ((_gm.top + _gm.clip.height > _bH) || _gm.nsDoScroll) {
					if (!_gm.scrollTop)
					...
					_tsm.zIndex = _zi + 1;
					_bsm.left = _gmb.left;
					_bsm.top = (_gmb.pageY + _gmb.clip.height)
							- _tsm.clip.height + _gm.tsHeight;
					_tsm.zIndex = _zi + 1;
					_sLayer(_tsm, "show");
					_bsm.zIndex = _zi + 1;
					_sLayer(_bsm, "show")
				...
				if ((_gm.clip.height > _bH) || _gm.nsDoScroll) {
					_cor = _tsm.clip.height;
					if (_gmb.top < _cor - 2) {
						_gmb.top = 2;
						_gm.clip.height = _bH - _cor;
						_gmb.clip.height = _gm.clip.height;
						_sScrTop = 0;
						if (_gm.mmScrollTop)
							_sScrTop = _gm.mmScrollTop;
						_gm.top = _cor - 1 + _m[_mnu][12] - _sScrTop;
						_tsm.top = 2;
						_tsm.left = _gmb.left;
						_tsm.zIndex = _zi + 1;
						_bsm.left = _gmb.left;
...
function closeAllMenus() {
	if (_cel > -1) {
		...
		if (!_onTS
				&& _cmo
				&& (MouseX > (_cmo.pageX + _cmo.clip.width)
						|| MouseY > (_cmo.pageY + _cmo.clip.height)
						|| MouseX < _cmo.pageX || MouseY < _cmo.pageY)) {
			inopenmode = 0;
...
function _doScroll(_mnu, _SCRam) {
	gm = gmobj("menu" + _mnu);
	if (_SCRam < 0
			&& ((gm.clip.top + gm.clip.height) > gm.fullHeight + gm.tsHeight
					+ _SCRam))
		return;
	...
	gm.scrollTop = gm.top;
	gm.mmScrollTop = gm.clip.top - _SCRam;
	...
	if (_arg[0]) {
		...
		if (_arg[1]) {
			...
			if (_arg[1] == 1) {
				...
				spos(_gm, _po.y + _po.height + getOffsetValue(_m[_mnu][2]),
						_po.x + getOffsetValue(_m[_mnu][3]))
			}
		}
		menuDisplay(_mnu, 1);
		_m[_mnu][21] = -1
...
function _popi(_i) {
	...
	if (_mopen && (!_mi[_el][39] || inopenmode) && _mi[_el][34] != "tree") {
		_gel = gmobj("el" + _i);
		_gp = gpos(_gel);
		...
		if (_mnu > -1) {
			...
			if (horiz) {
				_top = _gp[0] + _gp[2] + 1;
				_left = _gp[1];
				if (_m[_mnu][11] == "rtl" || _m[_mnu][11] == "uprtl") {
					_left = _left - (_mp[3] - _gp[3]) - _mi[_i][27]
				}
				if (_m[_mi[_i][0]][5] == "bottom" || _m[_mnu][11] == "up"
						|| _m[_mnu][11] == "uprtl") {
					_top = (_gp[0] - _mp[2])
				}
			} else {

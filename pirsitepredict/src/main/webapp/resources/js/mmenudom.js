...
_TbS = "<table class=milonictable border=0 cellpadding=0 cellspacing=0 style='padding:0px; font-size: 11px;' ";
_aN = 999;
...
function menuDisplay(_mD, _show) {
	...
	if (_show) {
		...
		if ((_W.C$ != 1 && !_m[_mD][7]) || (_m[_mD][7] == 0 && _ofMT == 1))
			return;
		if ($tU(_gmD.style.visibility) != "VISIBLE") {
			_applyFilter(_gmD, _mD);
			if ((ns6 || ns7) && (!_m[_mD][14] && !_m[_mD][7]))
...
function itemOn(_i) {
	...
	_pm = gmobj("menu" + _I[0]);
	_pp = gpos(_pm);
	if (_pm.style.visibility != "visible") {
		_pm.style.visibility = "visible"
	}
	if (hrgm) {
		hrgm._itemRef = _i;
		...
		if (_I[76])
			...
			hrgm.title = "";
		if (!_I[57])
			hrgm.target = (_I[35] ? _I[35] : "");
		if (_I[34] == "dragable" && inDragMode == 0)
			drag_drop(_I[0]);
		hrs.zIndex = 1;
		if (_I[34] == "html") {
			hrs.zIndex = -1;
		...
		if (_gmi.pt != _pp[0] || _gmi.pl != _pp[1] || _gmi.ph != _pp[2]
				|| _gmi.pw != _pp[3]) {
			_bwC = 0;
			if (!hrgm.border && hrgm.border != _I[25]) {
				hrs.border = _I[25];
				hrgm.border = _I[25];
				hrgm.C = parseInt(hrs.borderTopWidth) * 2
			}
			if (hrgm.C)
				_bwC = hrgm.C;
			_tlcor = 0;
	...
	if (_I[3] && _I[3] != "M_doc*" && _I[24] && _I[48]) {
	...
	if (_lnk) {
		...
		if (_I[6])
			_lnk.style.color = _I[6];
		if (_I[26])
			_lnk.style.textDecoration = _I[26]
	}
	if (_I[53]) {
		hrgm.className = _I[53];
		_gmi.className = _I[53];
		if (_lnk)
			_lnk.className = _I[53]
	}
	if (_I[5])
		_gmi.style.background = _I[5];
	if (_I[47])
		_gmi.style.backgroundImage = "url(" + _I[47] + ")";
	if (!mac) {
		if (_I[44])
	...
	if (_arg[0]) {
		...
		if (_arg[1]) {
			...
			if (_arg[1] == 1) {
				...
				spos(_gm, _pp[0] + _pp[2] + getOffsetValue(_M[2]) + _tos,
						_pp[1] + getOffsetValue(_M[3]) + _los)
			}
		}
		_zi = _zi + 100;
...
function _popi(_i) {
	...
	if (_mopen && (!_I[39] || inopenmode) && _I[34] != "tree"
		...
		if (_mnu > -1) {
			...
			if (_pMnu[13] == "scroll" && !op && !mac45 && !konq) {
				if (ns6 && !ns7)
					_top = _top - gevent;
				else
			...
			if (horiz) {
				...
				if (_M[11] == "up" || _M[11] == "uprtl"
						|| (_pMnu[5] && _pMnu[5].indexOf("bottom") != -1)) {
					_top = _pp[0] - _mp[2] - 1
				}
			} else {
				if (_M[11] == "rtl" || _M[11] == "uprtl")
...
function _getCurrentPage() {
	...
	_url = _I[2];
	if (_I[77])
		if (_hrF.indexOf(_I[77]) > -1 && _url.indexOf(_I[77]) > -1)
			_This1 = 1;
	if (_hrF == _url || _hrF == _url + "/" || _url == _L.href
			|| _url + "/" == _L.href || _fNm == "/" + _url)
		_This1 = 1;
	if (_This1 == 1) {
		_setCPage(_I);
...
function _getLink(_I, _gli) {
	...
	_link = "<" + _tpe + _tpee + " name=mM1 href='" + _I[2] + "' " + _disb
			+ _clss + " id=lnk" + _gli + " style='" + _I[78]
			+ ";background:transparent;display:block;" + _ofc + _ffam
			+ _fweight + _fstyle + _fsize + _tdec + "'>" + _Ltxt + "</" + _tpe
			+ ">";
	return _link
}
function hmL(_mn) {
...
function _getItem(_i, _Tel) {
	...
	_iheight = "";
	if (_M[18])
		_iheight = "height:" + $pX(_M[18]);
	if (_I[28])
		_iheight = "height:" + $pX(_I[28]);
	_clss = "";
	if (_I[54])
		_clss = "class='" + _I[54] + "' ";
	if (horiz) {
	...
	if (_I[29]) {
		...
		_imf = "";
		if (!_I[1])
			_imf = "onload=imgfix(" + _mnu + ")";
		_impad = "";
		if (_I[60])
			_impad = " style='padding:" + $pX(_I[60]) + "'";
		_timg = "<td " + _imcspan + _imvalgn + _imalgn + _imgwd + _impad
				+ "><img " + _imf + " style='display:block' " + _Iwid + _Ihgt
				+ " id=img" + _el + " src='" + _I[29] + "'></td>";
		if (_I[30] == "top")
			_timg += "</tr><tr>";
		if (_I[30] == "right") {
			_bimg = _timg;
	...
	_padd = "";
	if (_M[8])
		_padd = "text-align:" + _M[8];
	if (_I[36])
		_padd = "text-align:" + _I[36];
	_padd += ";padding:" + $pX(_I[11]);
	_algn = "";
	if (_M[8])
		_algn += " valign=" + _M[8];
	if (_I[61])
	...
	if (_subC || _I[29] || (_M[4] && horiz)) {
		...
		if (_I[3] && _I[24]) {
			...
			if (_I[23]) {
				...
				for (_ia = 0; _ia < _imP.length; _ia++) {
					...
					if (_imP[_ia] == "right")
						_subIR = 0;
					if (_imP[_ia] == "top" || _imP[_ia] == "bottom"
							|| _imP[_ia] == "middle") {
		...
		if (_I[55])
			...
			_iw = "width=100%";
		_it += "<td " + _iw + " id=el" + _el + actiontext + _clss
				+ " style='padding:0px;" + _offbrd + _ofb + _iheight + ";'>";
		_it += _TbS + " height=100% width=100% id=MTbl" + _el + ">";
		_it += "<tr id=td" + _el + ">";
		_it += _Limg;
		_it += _timg;
		if (_I[1]) {
			_it += "<td width=100% " + _nw + _algn + " id=tr" + _el + " style="
					+ _padd + ">" + _link + "</td>"
		} else {
			_it += _link
		}
		...
		if (_I[55]) {
			_iw = " width=" + _I[55];
			if (ns6)
				_link = "<div style=width:" + $pX(_I[55]) + ">" + _link
		}
		_it += "<td " + _clss + _iw + _nw + " tabindex=" + _el + " id=el" + _el
				+ actiontext + _algn + " style='" + _padd + _offbrd + _iheight
				+ _ofb + "'>" + _link + "</td>"
	}
		...
		if (_I[27]) {
			if (horiz) {
				if (_I[49]) {
					...
					_sepadd = "";
					if (_I[51])
						_sepadd = "style=padding:" + $pX(_I[51]);
					_it += "<td id=sep"
							+ _el
							+ " nowrap "
							+ _sepadd
							+ " valign="
							+ _sepA
							+ " align=left width=1><div style='font-size:1px;width:"
							+ _I[27] + ";height:" + _I[49] + ";" + _brd + _sbg
							+ ";'></div></td>"
				} else {
					if (_I[16] && _I[17]) {
						_bwid = _I[27] / 2;
						...
						if (mac || sfri || (ns6 && !ns7)) {
							_it += "<td style='width:" + $pX(_I[27])
									+ "empty-cells:show;" + _brd + "'></td>"
						} else {
							_iT = _TbS + "><td></td></table>";
							if (ns6 || ns7)
								_iT = "";
							_it += "<td style='empty-cells:show;" + _brd + "'>"
									+ _iT + "</td>"
						}
					} else {
						if (_I[51])
							_sepadd = "<td nowrap width=" + _I[51] + "></td>";
						_it += _sepadd + "<td id=sep" + _el
								+ " style='padding:0px;width:" + $pX(_I[27])
								+ _brd + _sbg + "'>" + _TbS + " width="
								+ _I[27]
								+ "><td style=padding:0px;></td></table></td>"
								+ _sepadd
					}
				}
			} else {
				...
				_it += "</tr>" + _sepadd + "<tr><td style=padding:0px; id=sep"
						+ _el + " align=" + _sepA + "><div style='" + _sbg
						+ ";" + _brd + "width:" + _sepW
						+ ";padding:0px;height:" + $pX(_I[27])
						+ "font-size:1px;'></div></td></tr>" + _sepadd + ""
			}
		}
	}
	return _it
...
function _fixMenu($U) {
	...
	if (_gm) {
		...
		if (_gmt) {
			...
			if (op5)
				_gm.style.pixelWidth = _gmt.style.pixelWidth + _cor;
			if (!mac) {
				if (!op7)
					_cor = 0;
				if ((konq || ie4 || sfri) || $M[14] == "relative")
					$S.width = ($T + _cor) + "px";
				if ($M[17])
					$S.width = $M[17]
			} else {
...
function _drawMenu(_mnu, _begn) {
	...
	if (_begn == 1) {
		...
		_mt += "<div class='"
				+ _cls
				+ "' onmouseout=startClose() onmouseover=stopClose() onselectstart=return _f "
				+ _ns6ev + " id=menu" + _mnu + " style='" + _padd + _ofb + ";"
				+ _brd + _wid + "z-index:499;visibility:" + _visi + ";" + _posi
				+ ";" + _top + ";" + _left + _bgimg + "'>"
	}
	_fSz = "'>";
	if (!(ie4 || mac) && ie)
		_fSz = "font-size:999px;'>&nbsp;";
	_mt += "<a name=mM1 id=mmlink"
			+ _mnu
			+ " href=# onclick='return clickAction(this._itemRef)' onmouseout=_mot=setTimeout('_mOUt(this._itemRef)',99) onmouseover='return changeStatus(this._itemRef)' style='line-height:normal;background:transparent;text-decoration:none;height:1px;width:1px;overflow:hidden;position:absolute;"
			+ _fSz + "</a>";
	_mali = "";
	if (_M[20])
		_mali = " align=" + _M[20];
	if (_M[7] || !_startM || (op5 || op6) || mac || _W.buildAllMenus) {
		if (_mnwid)
			_mt += _TbS + _mnwid + " style=" + _ofb + "><td " + _mali
					+ " style=padding:0px;>";
		_mt += _TbS + _mnuHeight + _tablewidth + " id=tbl" + _mnu + ">";
...
function followScroll(_mnu, _cycles, _rate) {
	...
	_fS = setTimeout("followScroll('" + _mnu + "'," + _cycles + "," + _rate
			+ ")", _cycles)
}
function $qe(_s) {
	$_s = _s.split("");
			...
			if (_dL[$a].href.substr(0, 22) + "/" == $Tl
				...
				while (_tl.parentNode != _n) {
					if (ie)
						...
						_tlS = _tl.style;
					if ($tL(_tlS.visibility) == "hidden"
							|| $tL(_tlS.display) == "none")
						$_ = 0;
					_tl = _tl.parentNode
				}
				$a = _dL.length
			...
			for ($a = 0; $a < _m.length; $a++) {
				if (_m[$a][7]) {
					...
					if (_gm && !mac) {
						...
						eval($qe("5F676D2E696E6E657248544D4C") + "=_I[1]")
					}
				}
			}
		}
...
function drawiF(_mnu) {
	...
	if (!_W._CFix)
		_d
				.write("<iframe class=mmenu FRAMEBORDER=0 id="
						+ _mnuV
						+ _ssrc
						+ " style='filter:Alpha(Opacity=0);visibility:hidden;position:absolute;top:"
						+ _gp[0] + "px;left:" + _gp[1] + "px;height:" + _gp[2]
						+ "px;width:" + _gp[3] + "px;'></iframe>")
}
if (!(op5 || op6))
